...
function retailfinancefield(executioncontext)// on change of Retail Finance field  Yes  //tab hide on opportunity 
{
		...
		if (deptValue == 1) {
			...
			function calculateInterestRates(executioncontext) {
				...
				function defaultpricelistthailand(executioncontext)// on load of opportunity setting defaul pricelist
				{
					...
					lookupvalue[0].id = "{af15368a-1b5d-ea11-a811-000d3aa3e47c}";
				...
				function defaultcurrency(executioncontext) {
					...
					lookupvalue[0].id = "{84c78aa6-883e-ea11-a812-000d3aa3e47c}";
	...
	function EMIcalculation(executioncontext) {
		...
		var interestrate = formContext.getAttribute("cnhisea_interestrate").getValue();
		formContext.getAttribute("cnhisea_finalinterestrate").setValue(interestrate);

		var finalinterestrate = formContext.getAttribute("cnhisea_finalinterestrate").getValue();
		...
		if (Retailfinancefield != null && Retailfinancefield == 1) {
			...
			var minimumdownpayment = Totalprice * (25 / 100);
			...
			var TractorInterestAmount = TractorFinanceAmount * FinancingYears * (finalinterestrate / 100);
			formContext.getAttribute("cnhisea_tractorinterestamount").setValue(TractorInterestAmount);
			...
			var FeeInterestAmount = (fee) * (finalinterestrate / 100) * FinancingYears;
			formContext.getAttribute("cnhisea_feeinterestamounts").setValue(FeeInterestAmount);
	...
	// Special Campaign Visibility
	function Specialcampaignvisibility(executionContext) {
		var formContext = executionContext.getFormContext();
		var HP = formContext.getAttribute("cnhisea_hpoftheselectedmodel").getValue();
		if (HP > 110) {
			formContext.getControl("cnhisea_specialcampaignrequired").setVisible(true);

		} else {
			formContext.getAttribute("cnhisea_specialcampaignrequired").setValue(false);
			formContext.getControl("cnhisea_specialcampaignrequired").setVisible(false);

		}

	}
	//Special Campaign EMI Calculation
	function specialcampaignrequired(executionContext) {
		var formContext = executionContext.getFormContext();
		var Scr = formContext.getAttribute("cnhisea_specialcampaignrequired").getValue();
		if (Scr == 1) {
			formContext.getControl("cnhisea_downpayment").setDisabled(true);
			formContext.getAttribute("cnhisea_downpayment").setValue(791850005);
			formContext.getAttribute("cnhisea_showtractorimplementemicalculationssectio").setValue(false);
			formContext.getControl("cnhisea_showtractorimplementemicalculationssectio").setDisabled(true);


			var SPtenor = formContext.getControl("cnhisea_tenor");
			SPtenor.removeOption(791850002);
			SPtenor.removeOption(791850003);
			SPtenor.removeOption(791850004);
			SPtenor.removeOption(791850005);
			...
			var Minimumdownpayment = Totalprice * (50 / 100);
			formContext.getAttribute("cnhisea_finaldownpayment").setValue(Minimumdownpayment);

			var finaldownpayment = formContext.getAttribute("cnhisea_finaldownpaymentz").getValue();
			var finaldownpaymentzpercent = (finaldownpayment / Totalprice) * 100;
			formContext.getAttribute("cnhisea_finaldownpaymentpercentage").setValue(finaldownpaymentzpercent);
			...
			var TractorInterestAmount = TractorFinanceAmount * FinancingYears * 0;
			formContext.getAttribute("cnhisea_tractorinterestamount").setValue(TractorInterestAmount);
			...
			formContext.getControl("cnhisea_downpayment").setDisabled(false);
			formContext.getControl("cnhisea_showtractorimplementemicalculationssectio").setDisabled(false);
	...
	function addoptionstotenorretailfinance(executionContext) {
		var formContext = executionContext.getFormContext();

		var Scr1 = formContext.getAttribute("cnhisea_specialcampaignrequired").getValue();
		if (Scr1 == 0) {
			//var SPtenor = formContext.getControl("cnhisea_tenor");
			var three = { value: 791850002, text: "3" };
			var four = { value: 791850003, text: "4" };
			var five = { value: 791850004, text: "5" };
			var six = { value: 791850005, text: "6" };

			formContext.getControl("cnhisea_tenor").addOption(three);
			formContext.getControl("cnhisea_tenor").addOption(four);
			formContext.getControl("cnhisea_tenor").addOption(five);
			formContext.getControl("cnhisea_tenor").addOption(six);
		}
